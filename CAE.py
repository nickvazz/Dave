...
ConAE.fit(X_train, X_train, epochs=20, batch_size=1, shuffle=True, validation_data=(X_train,X_train))
...
middle_layer = Model(inputs=ConAE.input,
                     outputs=ConAE.get_layer('code').output)
middle_layer_output = middle_layer.predict(X_test)

# print middle_layer_output
# print middle_layer_output.shape
# print middle_layer_output.reshape(len(X_train),2), temps

# X = middle_layer_output.reshape(len(middle_layer_output)/2,2)
# X2 = middle_layer_output2.reshape(len(middle_layer_output2)/2,2)
